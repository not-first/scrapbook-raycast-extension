...
export default function SearchUsers(props: LaunchProps) {
  ...
  return (
    <List isLoading={isLoading} searchText={searchText} onSearchTextChange={setSearchText} isShowingDetail throttle>
        <List.EmptyView title={searchText.length > 0 ? "No users found" : `${data?.length || 0} users loaded`} description={searchText.length > 0 ? "No users match the current search query" : "Begin typing to search"} icon={Icon.Person} />
